...
class Visualizer:
    ...
    def draw_instance_mask(self, predictions):
        """
        Draw only masks on image.
    ...
    def draw_instance_edges(self, predictions):
        """
        Draw only edges of masks on image.
    ...
    #copied to have edge color set to white
        num_instances = None
        if boxes is not None:
            boxes = self._convert_boxes(boxes)
            num_instances = len(boxes)
        ...
        for i in range(num_instances):
            ...
            if masks is not None:
                for segment in masks[i].polygons:
                    self.draw_polygon_mask(segment.reshape(-1, 2))
    ...
    #copied to have mask color set to black
        num_instances = None
        if boxes is not None:
            boxes = self._convert_boxes(boxes)
            num_instances = len(boxes)
        ...
        for i in range(num_instances):
            ...
            if masks is not None:
                for segment in masks[i].polygons:
                    self.draw_polygon_edges(segment.reshape(-1, 2))
    ...
    def draw_polygon_mask(self, segment):
        """
        added by lulu : draw white polygons masks
        Args:
            segment: numpy array of shape Nx2, containing all the points in the polygon.
        Returns:
            output (VisImage): image object with polygon drawn.
        ...
        polygon = mpl.patches.Polygon(
            ...
            linewidth=0, #set edges width to zero for mask
        )
        self.output.ax.add_patch(polygon)
        return self.output
    
    def draw_polygon_edges(self,segment):
        """
        added by lulu : draw outlines of polygons
        Args:
            segment: numpy array of shape Nx2, containing all the points in the polygon.
            edge_color: color of the polygon edges. Refer to `matplotlib.colors` for a
                full list of formats that are accepted. If not provided, a darker shade
                of the polygon color will be used instead.
        Returns:
            output (VisImage): image object with edges drawn.
        """
        polygon = mpl.patches.Polygon(
            segment,
            fill=True,
